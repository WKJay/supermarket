...
public class QuestString {
	...
	public void querySql(String s) throws SQLException {
		...
		ResultSet resultset = DBConn.getResult(s);
		if (resultset.next())
	...
	public String pageFooter() {
		//创建一个表单
		String s = "<form action=" + httpFile + " name=formBean methord=post class='form-inline' style='text-align:center'>";
		...
		s = s + "&nbsp;&nbsp;转到<input type ='text' name='pages' class='form-control input-sm'>页"
				...
				+ "'><button type='submit' class='btn btn-primary  btn-xs'>GO</button></form>";
		return s;
	}
	  /**
