...
<jsp:useBean id="calendar" scope="page" class="com.infinity.showtime.ShowTime"></jsp:useBean>
  ...
  <body>
  	<%
  		...
  		String rkbh			= null;
  		String maxCount     = "select max(id) from tb_ruku";
  	 ...
  	 <blockquote class="layui-elem-quote">
		<span  class="word_deepgrey"> 当前位置：库存管理 ></span>商品入库 &gt;&gt;&gt;
	 </blockquote>
	 <br>
	 
	  <fieldset class="layui-elem-field" style="width:95% ;margin-left:15">
	  <legend>入库商品选择及核验</legend>
	  <div class="layui-field-box">
		<form name="form1" method="post" action="sprk.jsp" class="layui-form"  >
		...
		<tr>
			...
			<td width = "178"  bgcolor="#EEEEEE">
				<div class="layui-input-inline">
					<select name="rkspname" lay-verify="required" lay-search=""  lay-filter="rkspname">
						 <option value="" selected="selected" disabled>选择入库商品</option>
		                 <%while(rs.next()){%>
			...
			<td width = "178" bgcolor="#EEEEEE">		
				  <input type="text" class="form-control" name="rkbh" 
				  value = "<%=rkbh %>"
	  ...
	  <legend>入库</legend>
	  <div class="layui-field-box">
		<form name="form2" method="post" action="change/spChange.jsp" class="layui-form">
		 <%
		 request.setCharacterEncoding("UTF-8");
		...
		<tr>
			...
			<td width = "178" bgcolor="#EEEEEE">
				<input class="form-control input-sm" type="text" name="yf" readonly>
			</td>
		</tr>
		...
		<tr>
			...
			<td width = "178" bgcolor="#EEEEEE">
				<div class="input-group">
				  ...
				  <input type="text" class="form-control" name="sf" aria-describedby="basic-addon1" onBlur="autoje()">
			    </div>
			</td>
			...
			<td width = "178" bgcolor="#EEEEEE">
				<input class="form-control input-sm" type="text" name="wf" readonly>
			</td>
		</tr>
		...
		<tr>
			...
			<td width = "178" bgcolor="#EEEEEE">
				<input class="form-control input-sm" type="text" name="rkdate" readonly 
...
function autoje()
{
    ...
    if(form2.sf.value-form2.yf.value>0){
      ...
      layer.msg('实付金额不应大于应付金额!');form2.sf.focus();return;
    }
}
...
function check(){
	...
	if(form2.sf.value==0){
		layer.msg('请输入实付金额');form2.sf.focus();return;
	}
...
function showcheck2()
{
	layer.open({
	  ...
	  ,content: 
	  	'<div style="text-align:center; padding: 10px; line-height: 22px; background-color: #393D49; color: #fff; font-weight: 300;">请确认商品入库信息</div>'
	  	+'<div style="padding:6px "> <table class="layui-table"  >'
	  	//第一行
	  	+'<tr>'
	  		+'<td id="td2">入库编号</td>'
	  		+'<td id="td2">'+form1.rkbh.value+'</td>'
	  	...
	  	+'<tr>'
	  		+'<td id="td2">实付</td>'
	  		+'<td id="td2">'+form2.sf.value+'</td>'
	  		
	  		+'<td id="td2">应付</td>'
	  		+'<td id="td2">'+form2.yf.value+'</td>'
	  	+'<tr>'
	  	
	  	//第七行
	  	+'<tr>'
	  		+'<td id="td2">未付</td>'
	  		+'<td id="td2">'+form2.wf.value+'</td>'
